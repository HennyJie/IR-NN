        ...
        for rid, ranker in algorithms:
            for dataset in datasets:
                for metric in p:
                    ...
                    f.write(f'java -jar RankLib-2.9.jar -train {dataset}/{fold}/train.txt -test {dataset}/{fold}/test.txt -validate {dataset}/{fold}/vali.txt -ranker {rid} -metric2t {metric} -metric2T {metric} -save model/{dataset}.{fold}.{ranker}.{metric}.Baseline.txt > performance_results/{dataset}.{fold}.{ranker}.{metric}.PerformanceResults.txt\n')
                    ...
                    f.write(f'java -jar RankLib-2.9.jar -train {dataset}/{fold}/square_train_with_newfeatures.txt -test {dataset}/{fold}/square_test_with_newfeatures.txt -validate {dataset}/{fold}/square_vali_with_newfeatures.txt -ranker {rid} -metric2t {metric} -metric2T {metric} -save model/{dataset}.{fold}.{ranker}.{metric}.txt > performance_results/{dataset}.{fold}.{ranker}.{metric}.SquareNewFeatures.PerformanceResults.txt\n')
                    ...
                    f.write(
                        f'java -jar RankLib-2.9.jar -load model/{dataset}.{fold}.{ranker}.{metric}.txt -test {dataset}/{fold}/square_test_with_newfeatures.txt -metric2T {metric} -idv output/{dataset}.{fold}.{ranker}.{metric}.output/{dataset}.{fold}.{ranker}.{metric}.txt\n')
